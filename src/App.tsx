...
import { ArrowsProvider, ArrowsLayer, useArrows } from './arrows'
...
/* ── card drag with arrows following + drop indicator ── */
...
function CardDragLayer({
  ...
  invalidateArrowsRef: React.MutableRefObject<(() => void) | null>
  onDrop: (taskId: string, targetIso: string, index: number) => void
  weekNode: HTMLDivElement | null
  cardDrag: DropIndicator | null
  setCardDrag: (d: DropIndicator | null) => void
}) {
  return null
}
...
function AppInner() {
  ...
  const cardDragRef = useRef<{ taskId: string; el: HTMLElement; startX: number; startY: number; dragging: boolean; origTransition: string; origZIndex: string } | null>(null)
  const suppressClick = useRef(false)
  const invalidateArrowsRef = useRef<(() => void) | null>(null)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [list, linkList, cols, tagList, folderList] = await Promise.all([
        fetchTasks(), fetchLinks(), fetchColumns(), fetchTags(), fetchFolders(),
      ])
      setTasks(list)
      setLinks(linkList)
  ...
  useEffect(() => {
    apiMe()
      .then((u) => { setUser(u); if (u) return loadData() })
  ...
  useEffect(() => {
    try { localStorage.setItem('planner.numDays', String(numDays)) } catch {}
  }, [numDays])
  ...
  const openTasksOf = useCallback(
    (iso: string) => tasks.filter((t) => t.date === iso && !t.done && matchesFilter(t)).sort((a, b) => a.order - b.order),
    [tasks, matchesFilter],
  )
  ...
  const multiDayNotes = useMemo(() => tasks.filter((t) => t.type === 'note' && t.startDate && t.endDate && t.startDate !== t.endDate), [tasks])

  const spanNotesForDay = useCallback(
    (iso: string) => multiDayNotes.filter((t) => t.startDate! <= iso && t.endDate! >= iso).sort((a, b) => {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      w.scrollLeft += (e.deltaY || e.deltaX) * factor
    }
  ...
  const handleRegister = useCallback(async (login: string, password: string) => {
  ...
  const handleLogout = useCallback(async () => {
    ...
    setUser(null); setTasks([]); setLinks([]); setTags([]); setActiveTags([])
    setColumnColors({}); setFolders([]); setSelectedFolder(null)
  }, [])

  /* ── folders ── */

  const handleCreateFolder = useCallback(async (name: string) => {
    try { const f = await createFolder(name); setFolders((prev) => [...prev, f]) } catch (e) { setError(String(e)) }
  }, [])

  const handleRenameFolder = useCallback(async (id: number, name: string) => {
    try { const f = await renameFolder(id, name); setFolders((prev) => prev.map((x) => (x.id === id ? f : x))) } catch (e) { setError(String(e)) }
  }, [])

  const handleDeleteFolder = useCallback(async (id: number) => {
    try { await deleteFolder(id) } catch (e) { setError(String(e)); return }
  ...
  const handleAssignFolder = useCallback(async (taskId: string, folderId: number | null) => {
    ...
    try { await updateTask(taskId, { folderId }) } catch (e) { setError(String(e)) }
  }, [])
  ...
  const handleToggle = useCallback(async (task: Task) => {
    ...
    try { await updateTask(task.id, { done: !task.done }) } catch (e) { setError(String(e)) }
  }, [])
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try { await deleteTask(id) } catch (e) { setError(String(e)) }
  }, [links])
  ...
  const handleEdit = useCallback(async (id: string, html: string) => {
    ...
    try { await updateTask(id, { text }) } catch (e) { setError(String(e)) }
  }, [])

  const handleCheckToggle = useCallback(async (task: Task, index: number, checked: boolean) => {
    ...
    try { await updateTask(task.id, { text, done }) } catch (e) { setError(String(e)) }
  }, [])

  /* ── drop task ── */

  const handleDropTask = useCallback(async (taskId: string, targetIso: string, index: number) => {
    const dragged = tasks.find((t) => t.id === taskId)
    if (!dragged) return
    const dayOpen = tasks.filter((t) => t.date === targetIso && !t.done && t.id !== taskId).sort((a, b) => a.order - b.order)
    ...
    try { await batchUpdate(updates) } catch (e) { setError(String(e)) }
  }, [tasks])

  /* ── card drag ── */

  const findDropIndex = useCallback((body: HTMLElement, clientY: number, excludeId: string): number => {
    const rows = Array.from(body.querySelectorAll<HTMLElement>('.task:not(.done)')).filter((r) => r.dataset.taskId !== excludeId)
  ...
  const handleCardPointerDown = useCallback(
    (e: React.PointerEvent, task: Task) => {
      ...
      cardDragRef.current = { taskId: task.id, el, startX, startY, dragging: false, origTransition: el.style.transition, origZIndex: el.style.zIndex }
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        d.el.style.boxShadow = '0 8px 32px rgba(0,0,0,0.18)'
        ...
        const over = document.elementsFromPoint(ev.clientX, ev.clientY).find((x) => (x as HTMLElement).classList?.contains('day')) as HTMLElement | null
        const iso = over?.dataset.date ?? null
        if (iso) {
          const body = over.querySelector('.day-body')
          if (body) {
            const idx = findDropIndex(body, ev.clientY, d.taskId)
            setDropIndicator({ iso, index: idx })
          }
        } else {
          setDropIndicator(null)
        }

        invalidateArrowsRef.current?.()
      }
      ...
      const onUp = (ev: PointerEvent) => {
        ...
        setDropIndicator(null)
        if (d?.dragging) {
          d.el.style.transition = d.origTransition
          d.el.style.zIndex = d.origZIndex
          ...
          const over = document.elementsFromPoint(ev.clientX, ev.clientY).find((x) => (x as HTMLElement).classList?.contains('day')) as HTMLElement | null
          const iso = over?.dataset.date
          if (iso) {
            const body = over.querySelector('.day-body')
            const index = body ? findDropIndex(body, ev.clientY, d.taskId) : 0
    ...
    [weekNode, handleDropTask, findDropIndex],
  )
  ...
  const handleCreateLink = useCallback(async (from: string, to: string) => {
    ...
    try { const link = await createLink(from, to); setLinks((prev) => [...prev, link]) } catch (e) { setError(String(e)) }
  }, [links])
  ...
  const handleUpdateLink = useCallback(async (id: string, patch: Partial<Pick<Link, 'from' | 'to' | 'style'>>) => {
    ...
    try { await updateLink(id, patch) } catch (e) { setError(String(e)) }
  }, [links])

  const handleRemoveLink = useCallback(async (id: string) => {
    setLinks((prev) => prev.filter((l) => l.id !== id))
    try { await deleteLink(id) } catch (e) { setError(String(e)) }
  }, [])

  /* ── gantt ── */

  const collectTaskTree = useCallback((id: string) => {
    const byParent = new Map<string, string[]>()
    for (const t of tasks) { const p = t.parentId ?? ''; if (p) { const arr = byParent.get(p) ?? []; arr.push(t.id); byParent.set(p, arr) } }
    const ids = new Set([id])
    const stack = [id]
    while (stack.length > 0) {
      const cur = stack.pop()!
      for (const c of byParent.get(cur) ?? []) { if (!ids.has(c)) { ids.add(c); stack.push(c) } }
    }
    return ids
  }, [tasks])

  const handleDeleteTree = useCallback(async (id: string) => {
  ...
  const handleGanttCreate = useCallback(async (text: string, start: string, end: string, parentId: string | null, progress: number): Promise<Task> => {
    try {
  ...
  const handleGanttUpdate = useCallback((id: string, patch: Partial<Pick<Task, 'text' | 'startDate' | 'endDate' | 'progress' | 'parentId'>>) => {
    setTasks((prev) => prev.map((t) => (t.id === id ? { ...t, ...patch } : t)))
    void updateTask(id, patch).catch((e) => setError(String(e)))
  }, [])

  const handleGanttBatch = useCallback(async (items: BatchUpdate[]) => {
    const byId = new Map(items.map((i) => [i.id, i]))
    setTasks((prev) => prev.map((t) => {
      const u = byId.get(t.id)
      if (!u) return t
      return { ...t, ...(u.date !== undefined ? { date: u.date } : {}), ...(u.order !== undefined ? { order: u.order } : {}), ...(u.startDate !== undefined ? { startDate: u.startDate } : {}), ...(u.endDate !== undefined ? { endDate: u.endDate } : {}), ...(u.progress !== undefined ? { progress: u.progress } : {}) }
    }))
    try { await batchUpdate(items) } catch (e) { setError(String(e)) }
  }, [])
  ...
  const handleColumnColor = useCallback(async (iso: string, color: string | null) => {
    setColumnColors((prev) => { const next = { ...prev }; if (color) next[iso] = color; else delete next[iso]; return next })
    try { if (color) await apiSetColumnColor(iso, color); else await apiClearColumnColor(iso) } catch (e) { setError(String(e)) }
  }, [])
  ...
  const handleUpdateStyle = useCallback(async (id: string, style: CardStyle | null) => {
    ...
    try { await updateTask(id, { style }) } catch (e) { setError(String(e)) }
  }, [])

  const handleUpdateTags = useCallback(async (id: string, nextTags: string[]) => {
    setTasks((prev) => prev.map((t) => (t.id === id ? { ...t, tags: nextTags } : t)))
    try { await updateTask(id, { tags: nextTags }) } catch (e) { setError(String(e)) }
    const merged = new Set(tags); for (const tg of nextTags) merged.add(tg)
    setTags([...merged].sort())
  }, [tags])
  ...
  const handleAddTag = useCallback(async (name: string) => {
    try { const list = await createTag(name); setTags(list) } catch (e) { setError(String(e)) }
  }, [])
  ...
  const handleRenameTag = useCallback(async (oldName: string, name: string) => {
    try {
      ...
      setTags(list)
      setTasks((prev) => prev.map((t) => (t.tags ?? []).includes(oldName) ? { ...t, tags: (t.tags ?? []).filter((tg) => tg !== oldName).concat(t.tags?.includes(name) ? [] : [name]) } : t))
  ...
  const { linkedSet, edgeSet } = useMemo(() => {
    ...
    const queue = [activeId]; linked.add(activeId)
    while (queue.length > 0) {
      const id = queue.pop()!
      for (const nb of adj.get(id) ?? []) { if (!linked.has(nb)) { linked.add(nb); queue.push(nb) } }
    }
    const indeg = new Map<string, number>(), outdeg = new Map<string, number>()
    for (const l of links) { outdeg.set(l.from, (outdeg.get(l.from) ?? 0) + 1); indeg.set(l.to, (indeg.get(l.to) ?? 0) + 1) }
    for (const id of linked) { const i = indeg.get(id) ?? 0, o = outdeg.get(id) ?? 0; if (i + o > 0 && (i === 0 || o === 0)) edge.add(id) }
  ...
  const toolbar = <FormatToolbar onCommand={runCommand} disabled={view === 'gantt'} />

  /* ── render ── */

  return (
    <ArrowsProvider
      weekNode={view === 'week' ? weekNode : view === 'rows' ? rowsNode : listNode}
      links={links}
      onCreate={handleCreateLink}
      onUpdate={handleUpdateLink}
      onRemove={handleRemoveLink}
      invalidateRef={invalidateArrowsRef}
    >
      {!authChecked ? (
        <div className="login-screen"><div className="status">Проверка сессии…</div></div>
      ) : !user ? (
                  ...
                  {days.map((day, idx) => {
                    ...
                    return (
                      <DayColumn
                        ...
                        linkedSet={linkedSet}
                        edgeSet={edgeSet}
                        dropIndex={dropIndicator?.iso === iso ? dropIndicator.index : null}
